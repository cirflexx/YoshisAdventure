class Game implements Subject {
    ...
    private checkCollision(): void {
        ...
        for (let enemy of this.collisionArray) {
            ...
            // Als powerup actief is wordt gechecked of yoshi tegen powerup aan komt.
            if (this.powerup) {
                if (Utils.checkCollision(this.yoshi, this.powerup)) {
                    //Powerup wordt verwijderd
                    this.powerup.div.remove();

                    //Alle vijanden die gesubscribed zijn wordt de notify functie afgevuurd.
    ...
    private removeEgg(egg) {
        // Het ei (die bosts of uit t scherm gaat) wordt verwijderd.
        let i: number = this.eggCollection.indexOf(egg);
        if (i != -1) {
    ...
    private addScore() {
        //Als een vijand dood is krijgt de speler 10 punten erbij.
        this.score += 10;
    }
    ...
    private createEnemiesOnScore() {
        ...
        // Als de powerup in het spel wordt gezet worden deze getekent.
        if (this.spawn150) {
            this.powerup.draw();
        }
